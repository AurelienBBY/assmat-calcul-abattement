...
- **Hébergement (décision 2026-07-19)** : sous-domaine de l'hébergement existant de l'utilisateur (ex. `abattement.<domaine>.fr`), en **statique pur** — le serveur ne fait que servir les fichiers, aucune donnée n'y transite. Prérequis : HTTPS (Let's Encrypt) sur le sous-domaine, indispensable au service worker. Déploiement = upload du dossier (script FTP/rsync simple). GitHub Pages reste le plan B.
